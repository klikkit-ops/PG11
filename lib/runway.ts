...
export async function generateDancePrompt(
  ...
  const getFallbackPrompt = (): string => {
    ...
    return `A realistic, photorealistic pet ${petDescription ? `(${petDescription})` : ''} ${styleDescription} in a ${backgroundDescription}. The pet maintains its authentic, photographic appearance exactly as in the source image - preserving the original fur texture, colors, markings, and facial features. The pet's face, features, and unique identity are clearly preserved throughout the video. CRITICAL: The pet MUST remain FULLY VISIBLE and COMPLETELY IN FRAME at ALL TIMES - the entire pet body, head, and all features must always be visible, never cropped or cut off. MOST IMPORTANT: The pet must be INTEGRATED INTO and DANCING WITHIN the themed environment - the pet should appear physically present in the actual themed location, dancing on the dance floor or within the scene, NOT shown separately or in a split-screen format. The camera maintains appropriate distance to keep the full pet in frame throughout all dance movements. The dance movements are smooth, natural, and match the ${danceStyle} style accurately while keeping the pet centered and fully visible. The background is rich, detailed, and thematically appropriate - never plain or empty. The pet and environment must appear in the SAME unified scene together. Vertical 9:16 aspect ratio, high-quality photography, realistic appearance, well-lit environment, stable camera, full body visible, pet always in frame, 8-10 second duration. No cartoon style, no illustration, no plain backgrounds, no cropping of pet, no split-screen, no separate scenes, purely photorealistic unified composition.`;
  };
...
- MOST IMPORTANT: The pet must be INTEGRATED INTO and DANCING WITHIN the themed environment - NOT shown separately, NOT in a split-screen, NOT in a separate scene. The pet should appear to be physically present and dancing in the actual themed location (e.g., dancing on the disco dance floor, not shown separately from it)
- The background MUST be rich, detailed, and thematically appropriate - never plain, white, or empty
- The pet and the themed environment must appear in the SAME SCENE together - unified composition, not separate elements

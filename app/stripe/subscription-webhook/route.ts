...
async function handleCheckoutSessionCompleted(
  ...
  if (checkoutSession.mode === "subscription" && checkoutSession.subscription) {
    ...
    if (isTrial) {
      ...
      // Update subscription to use weekly price after trial period
      // The subscription was created with $0.49/week, we need to switch it to the weekly price
      // We'll update it now but with proration_behavior: 'none' so it doesn't charge until the next cycle
      ...
      if (weeklyPriceId && subscription.items.data.length > 0) {
        try {
          ...
          console.log(`[Webhook] Updated trial subscription ${subscription.id} to use weekly price ${weeklyPriceId} (will charge at next billing cycle)`);
        } catch (error) {
